...
| 提示行 | 今日已用金额 | 两行：**累计总 token** / **订阅剩余额度** |
...
| 订阅额度 | 无 | `/api/subscription/self`，多条 active 订阅合并求和，独占一行 |
| 气泡字号 | — | 整体缩小约 22%，避免四行文字撑出气泡 |

原项目的交互保留：拖拽 + 四边吸附、左吸附镜像翻转、按压 Q 弹、随窗口缩放、随机台词、汉堡菜单（大小 / 峰谷文案 / 气泡开关 / 每轮提示开关与自动关闭时间 / 避让滚动条）。

**已移除**的原项目功能：按压音效（含音量与音效包选择）、「用量模式」下拉（原本用于在余额记账与实时令牌之间切换，本 fork 数据源单一，该选项已无作用）。
