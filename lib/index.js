...
usageSelect.appendChild(soundOpt('ledger', '累计 Token (自动)'))
usageSelect.appendChild(soundOpt('token', '今日 Token (实时)'))
...
function buildTotalHint() {
  var parts = []
  parts.push('总 ' + (state.totalTokens !== null ? fmtToken(state.totalTokens) : '--'))
  if (state.subRemainCny !== null) parts.push('余 ' + fmtCny(state.subRemainCny))
  else if (state.subRemain !== null) parts.push('余 ' + fmtQuota(state.subRemain))
  return parts.join(' · ')
}
function render() {
  ...
  if (state.status === 'error') {
    ...
    hint = state.refreshing ? '正在同步全量…' : buildTotalHint()
  }
...
function apply(ctx) {
    ...
    function newapiAbsorb(items, acc) {
      for (const it of items || []) {
        ...
        bucket.tokens += (Number(it.prompt_tokens) || 0) + (Number(it.completion_tokens) || 0)
        bucket.requests += 1
    ...
    async function getBalancePayload() {
      ...
      return { ...payload, isPeak: newapiIsPeakNow(), usageMode: 'token' }
    }
