...
// v1 用日志 id 去重（错误，见 newapiAbsorb 注释），读到旧版本直接作废重建
const NEWAPI_CACHE_VERSION = 2
...
function apply(ctx) {
    ...
    // Modify by MHY, token 口径 = 实际处理量（与上游渠道计量一致），逐行按语义归一化：
    // - openai 语义（/v1/chat/completions、/v1/responses）：prompt_tokens 本身已含缓存
    // - anthropic 语义（/v1/messages）：prompt_tokens 不含缓存，缓存单独存 other.cache_tokens
    //   与 other.cache_creation_tokens / cache_write_tokens，需补加（上游 new-api 的已知
    //   双语义记账问题，见 text_quota.go 的 usage_semantic 分支）
    // 佐证：anthropic 行的 cache_tokens 可远大于 prompt_tokens（抽样最高 11419×），
    // 且缓存部分被计费表达式按 cr 档实收，确为真实处理的 token。
    function newapiRowTokens(it) {
      ...
      if (!o || o.usage_semantic !== 'anthropic') return pt + ct
      const cc = Number(o.cache_creation_tokens) || Number(o.cache_write_tokens) || 0
